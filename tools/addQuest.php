...
if (isset($_POST["userName"]) AND isset($_POST["password"]) AND isset($_POST["type"]) AND isset($_POST["amount"]) AND isset($_POST["reward"]) AND isset($_POST["questName"])) {
	...
	echo '<form action="addQuest.php" method="post">
		...
		Amount (How many orbs/coins/stars you need to collect): <input type="number" name="amount"><br>
		Reward (How many Diamonds you get as a reward): <input type="number" name="reward"><br>
		Quest Name: <input type="text" name="questName">
