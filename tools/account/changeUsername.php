...
if(isset($_POST["userName"]) AND isset($_POST["newUser"]) AND isset($_POST["password"])){
	...
	if ($pass == 1) {
		...
		if($query->rowCount() == 0){
			echo "Invalid password or non-existant account. <a href='changeUsername.php'>Try again.</a>";
		}else{
	...
	echo '<form action="changeUsername.php" method="post">Old username: <input type="text" name="userName"><br>New username: <input type="text" name="newUser"><br>Password: <input type="password" name="password"><br><input type="submit" value="Change"></form>';
}
?>