...
$query = $db->prepare("SELECT * FROM users WHERE userName = :userName OR userID = :userName ORDER BY stars DESC LIMIT 1");
$query->execute([':userName' => $playername]);
if ($query->rowCount() == 0) {
	$query = $db->prepare("SELECT * FROM users WHERE userName LIKE CONCAT(:str, '%') ORDER BY stars DESC LIMIT 1");
...
$query = $db->prepare("SELECT * FROM accounts WHERE accountID = :extID");
$query->execute([':extID' => $extid]);
$accinfo = $query->fetch();
$discordID = 0;
